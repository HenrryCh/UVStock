...
<h3> Devolucion Nro: {{ $devolucione->id}}</h3>
@if ($errors->any())
...
<div class="card card-primary">
  	...
  	<div class="card-body">
        ...
        <div class="form-group">
          <div class="col-lg-3">
              <label>Estado(*)</label>
              <button class="btn btn-{{$devolucione->estado ? 'warning' : 'success'}} btn-sm">@if ($devolucione->estado==1 or $devolucione->estado=="active") Pendiente @else Finalizado @endif</button>
          </div>
        </div>
      </div>
		</div>
                  ...
                  <th>Motivo Devolución</th>
				        ...
				        @foreach ($detalle_devoluciones as $key => $detalle_devolucione)
								<tr>
									...
									<td class="text-left">{{$detalle_devolucione->motivo_devolucione->nombre .' '.$detalle_devolucione->motivo_devolucione->estado}}</td>
								</tr>
								@endforeach
		...
		<div class="col-lg-12 col-md-12 col-sm-12 col-xs-12">
	        <div class="form-group text-right">
	           	<a href="{{URL::action('App\Http\Controllers\DevolucionesControlador@index')}}" class="btn btn-danger" title="Regresar al Listado Anterior">{{__('Volver')}}</a>
	        </div>
		</div>
	</div>
</div>
